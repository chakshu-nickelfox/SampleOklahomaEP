...
extension UICollectionView {
    ...
    func dequeueCell<T: NibNameProvider & StaticReuseIdentifierProvider>(_ headerType: T.Type, for indexPath: IndexPath) -> T? {
        guard let cell = self.dequeueReusableCell(withReuseIdentifier: headerType.reuseIdentifier, for: indexPath) as? T else {
            return nil
        }
        return cell
    }
...
extension UITableView {
    ...
    func dequeueCell<T: NibNameProvider & StaticReuseIdentifierProvider>(_ headerType: T.Type, for indexPath: IndexPath) -> T? {
        guard let cell = self.dequeueReusableCell(withIdentifier: headerType.reuseIdentifier, for: indexPath) as? T else {
            return nil
        }
        return cell
    }
